...
int get_clock(xeta::uint32 &clock_high, xeta::uint32 &clock_low, xeta::uint16 &ret_clock_seq)
{
	static int			adjustment = 0;
	static struct timeval		last = {0, 0};
	static xeta::uint16			clock_seq;
	struct timeval 			tv;
...
void xeta::uuid::read_from_buffer( unsigned char const* buffer)
{
...
void xeta::uuid::write_to_buffer( unsigned char * buffer) const
{
...
void xeta::uuid::generate_time()
{
	static unsigned char node_id[6];
	static int has_init = 0;
	xeta::uint32	clock_mid;
...
void xeta::uuid::to_buffer( unsigned char* buffer ) const
{
...
namespace xml
{
void read_from_string( const std::string & source, xeta::uuid& id  )
{
	using namespace boost::spirit;
	// write that parser!
	uint_parser<xeta::uint8, 16, 2, 2> hex_1;
	uint_parser<xeta::uint16, 16, 4, 4> hex_2;
...
void write_to_string( std::string & output, const xeta::uuid& id  )
{
	char out[37];
...
int main()
{
	...
	std::string content;
	xml::write_to_string( content, test );
	std::cout << content << "\"" << std::endl;
	
	xeta::uuid other_uuid;
	xml::read_from_string( content, other_uuid );
