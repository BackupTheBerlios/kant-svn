...
// Converted to C++ by Andreas Pokorny <apokorny@zynot.org>
...
void xeta::MD5::read_ctx( Sum& result )
{
	reinterpret_cast<uint32*>(result.data)[0] = SWAP (A);
	reinterpret_cast<uint32*>(result.data)[1] = SWAP (B);
	reinterpret_cast<uint32*>(result.data)[2] = SWAP (C);
	reinterpret_cast<uint32*>(result.data)[3] = SWAP (D);
}
...
IMPORTANT: On some systems it is required that RESBUF is correctly
aligned for a 32 bits value.  */
void xeta::MD5::finish_ctx ( Sum& result )
{
	...
	*reinterpret_cast<uint32*>(&internal_buffer[bytes + pad]) = SWAP (total[0] << 3);
	*reinterpret_cast<uint32*>(&internal_buffer[bytes + pad + 4]) = SWAP ((total[1] << 3) |
			(total[0] >> 29));
...
bool xeta::MD5::file( const char *filename, Sum& result )
{
...
/* Compute MD5 message digest for LEN bytes beginning at BUFFER.  The
   ...
   digest.  */
void xeta::MD5::buffer( const char *buffer, size_t len, Sum& result )
{
	init();
...
void xeta::MD5::process_bytes ( const char *buffer, size_t len )
{
	...
	if (len >= 64)
	{
		...
		/* To check alignment gcc has an appropriate operator.  Other
		   compilers don't.  */
# if __GNUC__ >= 2
#  define UNALIGNED_P(p) (long(p) % __alignof__ (uint32) != 0)
# else
#  define UNALIGNED_P(p) (long(p) % sizeof (uint32) != 0)
# endif
...
void xeta::MD5::process_block (char const* buffer,size_t len)
{
	uint32 const* words = reinterpret_cast<uint32 const*>(buffer);
...
void xml::read_from_string( std::string const& source, xeta::MD5::Sum & obj )
{
	using namespace boost::spirit;
	uint_parser<xeta::uint32, 16, 8, 8> hex_4;
	parse_info<const char *> p_info = parse(
			source.c_str(), 
			source.c_str() + source.length(),
			hex_4[ assign_a( reinterpret_cast<xeta::uint32*>(obj.data)[0] ) ] >> 
			hex_4[ assign_a( reinterpret_cast<xeta::uint32*>(obj.data)[1] ) ] >> 
			hex_4[ assign_a( reinterpret_cast<xeta::uint32*>(obj.data)[2] ) ] >> 
			hex_4[ assign_a( reinterpret_cast<xeta::uint32*>(obj.data)[3] ) ] 
			);
	if( !p_info.hit || p_info.length != 32 )
		throw std::runtime_error( ("Error while parsing md5sum: " + source).c_str() );
}

void xml::write_to_string( std::string & data, xeta::MD5::Sum const& obj )
{
	char buffer[33];
	for( int i = 0; i < 16; ++i )
	{
...
std::istream& operator>>( std::istream& input, xeta::MD5::Sum & object )
{
	...
	input >> source;
	try {
		xml::read_from_string( source, object );
		return input;
	}
...
std::ostream& operator<<( std::ostream& output, xeta::MD5::Sum const& object )
{
	for( int i = 0; i < 16; ++i )
	{
